  ...
  if (appointmentsQuery.isLoading)
    return (
      <p className="mt-4 text-sm text-neutral-500">
        Checking completed appointments...
      </p>
    );
  if (appointmentsQuery.isError)
    return (
      <p className="mt-4 text-sm text-red-500">
        We could not check your completed appointments.
      </p>
    );
  if (!appointmentsQuery.data?.length)
    return (
      <p className="mt-4 rounded-lg bg-primary-50 p-4 text-sm leading-6 text-neutral-500">
  ...
  return (
    <Formik<ReviewFormValues>
      ...
      onSubmit={(values, helpers) =>
        reviewMutation.mutate(values, { onSuccess: () => helpers.resetForm() })
      }
    >
      <Form className="mt-5 space-y-2">
        <SelectField
          ...
          options={appointmentsQuery.data.map((appointment) => ({
